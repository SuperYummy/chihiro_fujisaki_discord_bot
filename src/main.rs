...
use events::on_event;
use twilight_gateway::{stream::create_recommended, Config, ConfigBuilder, EventTypeFlags, Intents};
...
async fn main() -> Hope<()> {
    ...
    let chihiro_fujisaki = ChihiroFujisaki::summon();

    let chihiro_token = chihiro_fujisaki.token.to_owned();
